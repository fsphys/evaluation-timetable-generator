...
def build_timetable():
  ...
  for course_id, course in courses.items():
    ...
    if not found_date:
      print("ERROR: Did not find any date within the evaluation period for the course '{0}', LV-Nr. {1}!".format(course.name, course.lvnr))
...
def load_courses_from_filesystem():
  for file in glob.glob("{0}/*.json".format(json_directory)):
    ...
    for i, object in enumerate(data):
      course = Course(object['id'], object['no'], object['name'], object['lecturer'])
...
def course_parse_dates(dates):
  dates_list = []
  for date in dates:
    date_object = Appointment(parse_datetime(date["start_time"]), parse_datetime(date["end_time"]), date["room"])
    ...
    for course_id in timetable[weekday][block].keys():
      ...
      for otherweekday, otherblock in course.occurrences:
        if not (otherweekday == weekday and otherblock == block):
          print("      Auch {0}/{1}".format(weekdays_short[otherweekday], otherblock))
